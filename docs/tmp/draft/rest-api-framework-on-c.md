...
Архитектурно рекомендуется модульная система с конвейером middleware (как в Express/NestJS), чёткими правилами владения ресурсами (имитация «borrowing» из Rust), безопасной обработкой ошибок и управлением памятью (пулы/ареа-аллокатор), асинхронной моделью на основе epoll/ioh_uring (поддержка неблокирующего I/O, мультишот accept), и гибкими точками расширения (плагины, C-ABI для FFI). Приведены примеры API-дизайна на C23, учитывающие ownership-стиль и гарантии безопасности.
...
- **Сериализация/десериализация:** JSON (рекомендуется yyjson или jansson) и, по возможности, XML; поддержка Pooled-багова (cURL для шаблонов). Пример из IoHttp: `ioh_respond_json(resp, 200, "{\"ok\":true}")`【18†L378-L386】. _Сложность:_ низкая (готовые библиотеки).
...
- **Конвейер middleware:** По аналогии с Express/Koa, данные из запроса должны проходить цепочку функций: логирование → аутентификация → валидация → конечный обработчик. Это обеспечивает separation of concerns. _Пример на C23:_
  ```c
  typedef int (*ioh_handler_t)(ioh_request_t *req, ioh_response_t *resp, void *ctx);
  ioh_route_add(srv, IOH_POST, "/api/data", handler_data, ctx);
  ioh_middleware_add(srv, auth_middleware);
  ioh_middleware_add(srv, cors_middleware);
  ```
  Каждая middleware получает те же `req/resp` и может прервать цепочку, вызвав `ioh_respond_error`.
- **Ownership/borrowing-подобные паттерны:** В отсутствии Rust-библиотеки, документация должна явно прописывать, что фреймворк «владеет» ресурсами (записью в ответ) и передаёт их обработчику. Например, указатель `ioh_response_t *resp` владеется фреймворком; код-обработчик не должен его освобождать. Можно предусмотреть reference-count или scoped-блоки для буферов.
- **Error handling:** В C обычно возвращают коды ошибок. Можно оформить обобщённый `ioh_error_t` и использовать функции-помощники: `ioh_respond_json(resp, 500, "{\"error\":\"msg\"}")`. Рекомендуется отменить «longjmp»/sigsetjmp, полагаться на условные проверки. _Пример:_ проверка аргументов, и в случае ошибки `return ioh_respond_error(resp, 400, "invalid input");`.
- **Memory management:** Рекомендуется использовать пулы или ареа-аллокатор на время обработки одного запроса, а не индивидуальные `malloc`. После ответа освобождать пул (пример: arena_alloc для временных строк). Старайтесь избегать fragmention. _Пример:_ создайте `ioh_buffer_pool_t` для динамических данных, обнуляйте каждый новый запрос.
- **Асинхронная модель:** Базироваться на механизме epoll/kqueue/ioh_uring без внешнего цикла (EventLoop внутри). Как на IoHttp: multishot accept + SQPOLL. Предусмотреть неблокирующие API для чтения тела запроса и записи ответа. Не зависит от языка runtime – используйте низкоуровневые syscalls. _Пример:_
  ````c
...
- **Точки расширения:** Определить интерфейсы плагинов: например, возможность добавлять новые протоколы (gRPC, GraphQL) или фильтры. Предусмотреть регистрационные функции: `ioh_server_register_extension(srv, &my_plugin)`. Придерживайтесь принципа открытости/закрытости (Open/Closed principle).
- **FFI и биндинги:** Если фреймворк планируется использовать из других языков (например, Python через CFFI или Lua), обеспечить чистый C-API и C11. Например, структура `ioh_request_t` не должна включать C++ STL-контейнеры; используйте `extern "C"`. Можно создать генератор обёрток по аналогии с Python’s C API.
