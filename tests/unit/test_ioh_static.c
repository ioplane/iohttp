...
static void add_header(ioh_request_t *req, const char *name, const char *value)
{
    uint32_t idx = req->header_count;
    if (idx >= IOH_MAX_HEADERS) {
        return;
    }
...
{
    ioh_static_config_t cfg;
    ioh_static_config_init(&cfg);
    ...
    int rc = ioh_static_serve(&cfg, &req, &resp);
...
void test_static_etag_match(void)
{
    ...
    ioh_response_t resp2;
    TEST_ASSERT_EQUAL_INT(0, ioh_response_init(&resp2));

    rc = ioh_static_serve(&cfg, &req2, &resp2);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_EQUAL_UINT16(304, resp2.status);

    ioh_response_destroy(&resp2);
}
