...
void test_ws_compute_accept(void)
{
    ...
    char accept[IOH_WS_ACCEPT_KEY_LEN + 1];

    int rc = ioh_ws_compute_accept(key, accept);
...
{
    char accept[IOH_WS_ACCEPT_KEY_LEN + 1];
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_ws_compute_accept(nullptr, accept));
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_ws_compute_accept("key", nullptr));
}

void test_ws_validate_upgrade_valid(void)
{
    int rc =
        ioh_ws_validate_upgrade("GET", "websocket", "Upgrade", "dGhlIHNhbXBsZSBub25jZQ==", "13");
    TEST_ASSERT_EQUAL_INT(0, rc);
}

void test_ws_validate_upgrade_missing_key(void)
{
    int rc = ioh_ws_validate_upgrade("GET", "websocket", "Upgrade", "", "13");
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);
}

void test_ws_validate_upgrade_bad_method(void)
{
    int rc = ioh_ws_validate_upgrade("POST", "websocket", "Upgrade", "key==", "13");
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);
}

void test_ws_validate_upgrade_bad_version(void)
{
    int rc = ioh_ws_validate_upgrade("GET", "websocket", "Upgrade", "key==", "8");
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);
}
...
static ioh_ws_ctx_t make_ctx(test_io_t *io)
{
    return (ioh_ws_ctx_t){
        .on_msg = test_on_msg,
        ...
        .user_data = io,
        .wslay_ctx = nullptr,
        .max_recv_msg_len = IOH_WS_DEFAULT_MAX_MSG,
    };
}
...
void test_ws_ctx_init_destroy(void)
{
    ...
    ioh_ws_ctx_t ws = make_ctx(&io);

    int rc = ioh_ws_ctx_init(&ws);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_NOT_NULL(ws.wslay_ctx);

    TEST_ASSERT_TRUE(ioh_ws_want_read(&ws));
    TEST_ASSERT_FALSE(ioh_ws_close_received(&ws));
    TEST_ASSERT_FALSE(ioh_ws_close_sent(&ws));

    ioh_ws_ctx_destroy(&ws);
    TEST_ASSERT_NULL(ws.wslay_ctx);
}

void test_ws_ctx_init_null_recv(void)
{
    ioh_ws_ctx_t ws = {.recv = nullptr, .send = test_send};
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_ws_ctx_init(&ws));
}
...
void test_ws_send_text(void)
{
    ...
    rc = ioh_ws_queue_text(&ws, "World", 5);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_TRUE(ioh_ws_want_write(&ws));
...
void test_ws_close_handshake(void)
{
    ...
    TEST_ASSERT_TRUE(ioh_ws_close_received(&ws));
    TEST_ASSERT_TRUE(ioh_ws_close_sent(&ws));
...
void test_ws_queue_close(void)
{
    ...
    rc = ioh_ws_queue_close(&ws, 1001, "going away", 10);
    TEST_ASSERT_EQUAL_INT(0, rc);
