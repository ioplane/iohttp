...
void test_tls_config_validate_no_cert(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_tls_config_validate(&cfg));
}
...
void test_tls_ctx_create_destroy(void)
{
    ...
    ioh_tls_ctx_t *ctx = ioh_tls_ctx_create(&cfg);
    TEST_ASSERT_NOT_NULL(ctx);

    ioh_tls_ctx_destroy(ctx);

    /* nullptr is safe */
    ioh_tls_ctx_destroy(nullptr);
}
...
void test_tls_conn_create_destroy(void)
{
    ...
    ioh_tls_conn_t *conn = ioh_tls_conn_create(ctx, -1);
    TEST_ASSERT_NOT_NULL(conn);
    TEST_ASSERT_NOT_NULL(conn->ssl);
    TEST_ASSERT_NOT_NULL(conn->cipher_in_buf);
    TEST_ASSERT_NOT_NULL(conn->cipher_out_buf);
    TEST_ASSERT_EQUAL_size_t(IOH_TLS_CIPHER_BUF_SIZE, conn->cipher_in_cap);
    TEST_ASSERT_EQUAL_size_t(IOH_TLS_CIPHER_BUF_SIZE, conn->cipher_out_cap);
    TEST_ASSERT_FALSE(conn->handshake_done);

    ioh_tls_conn_destroy(conn);
    ioh_tls_ctx_destroy(ctx);

    /* nullptr is safe */
    ioh_tls_conn_destroy(nullptr);

    /* nullptr ctx returns nullptr */
    TEST_ASSERT_NULL(ioh_tls_conn_create(nullptr, -1));
}
...
void test_tls_feed_input_basic(void)
{
    ...
    int ret = ioh_tls_feed_input(conn, data, sizeof(data));
    ...
    ret = ioh_tls_feed_input(conn, more, sizeof(more));
    TEST_ASSERT_EQUAL_INT(0, ret);
    TEST_ASSERT_EQUAL_size_t(sizeof(data) + sizeof(more), conn->cipher_in_len);

    /* Null args */
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_tls_feed_input(nullptr, data, sizeof(data)));
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_tls_feed_input(conn, nullptr, sizeof(data)));

    /* Zero length is ok */
    TEST_ASSERT_EQUAL_INT(0, ioh_tls_feed_input(conn, data, 0));
...
void test_tls_handshake_self_signed(void)
{
    ...
    /* Server side: use our ioh_tls_ctx */
    ioh_tls_config_t scfg;
    ioh_tls_config_init(&scfg);
    scfg.cert_file = TEST_SERVER_CERT;
    scfg.key_file = TEST_SERVER_KEY;

    ioh_tls_ctx_t *sctx = ioh_tls_ctx_create(&scfg);
    TEST_ASSERT_NOT_NULL(sctx);

    ioh_tls_conn_t *sconn = ioh_tls_conn_create(sctx, sv[0]);
    ...
    for (int i = 0; i < 100; i++) {
        if (sret != WOLFSSL_SUCCESS) {
            ...
            if (sret != WOLFSSL_SUCCESS) {
                ...
                if (serr != WOLFSSL_ERROR_WANT_READ && serr != WOLFSSL_ERROR_WANT_WRITE) {
        ...
        if (cret != WOLFSSL_SUCCESS) {
            ...
            if (cret != WOLFSSL_SUCCESS) {
                ...
                if (cerr != WOLFSSL_ERROR_WANT_READ && cerr != WOLFSSL_ERROR_WANT_WRITE) {
                         ...
                         const char *expected_proto)
{
    ...
    int rc = wolfSSL_UseALPN(cssl, (char *)client_alpn, (unsigned int)strlen(client_alpn),
    ...
    const char *proto = ioh_tls_get_alpn(sconn);
