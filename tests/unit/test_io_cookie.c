...
void test_cookie_serialize_simple(void)
{
    io_cookie_t cookie = {
        ...
        .value = "abc123",
        .max_age = -1,
        .same_site = IO_SAME_SITE_DEFAULT,
    };

    char buf[256];
    int n = io_cookie_serialize(&cookie, buf, sizeof(buf));
...
/* ---- io_cookie_validate_name ---- */

void test_cookie_validate_name_valid(void)
{
    TEST_ASSERT_EQUAL_INT(0, io_cookie_validate_name("session"));
    TEST_ASSERT_EQUAL_INT(0, io_cookie_validate_name("__Host-id"));
    TEST_ASSERT_EQUAL_INT(0, io_cookie_validate_name("my_cookie_123"));
}

void test_cookie_validate_name_rejects_empty(void)
{
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_validate_name(nullptr));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_validate_name(""));
}

void test_cookie_validate_name_rejects_special_chars(void)
{
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_validate_name("a=b"));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_validate_name("a;b"));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_validate_name("a b"));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_validate_name("a\tb"));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_validate_name("a\nb"));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_validate_name("a\rb"));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_validate_name("\x01name"));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_validate_name("del\x7F"));
}
...
{
    char buf[256];
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_serialize(nullptr, buf, sizeof(buf)));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_cookie_serialize(nullptr, nullptr, 0));
}
...
void test_ctx_set_cookie(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, io_ctx_set_cookie(&ctx, &cookie));
    ...
    TEST_ASSERT_EQUAL_INT(0, io_ctx_set_cookie(&ctx, &cookie2));
