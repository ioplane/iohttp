/**
 * @file test_io_vary.c
 * @brief Unit tests for ioh_response_add_vary() Vary header accumulator.
 */
...
static const char *find_vary_value(const ioh_response_t *r)
{
...
void test_vary_multiple_tokens(void)
{
    ...
    rc = ioh_response_add_vary(&resp, "Accept");
    TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_vary_no_duplicate(void)
{
    ...
    rc = ioh_response_add_vary(&resp, "Accept-Encoding");
    TEST_ASSERT_EQUAL_INT(0, rc);
...
{
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_response_add_vary(nullptr, "Accept"));
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_response_add_vary(&resp, nullptr));
}

void test_vary_empty_token(void)
{
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_response_add_vary(&resp, ""));
}
...
void test_vary_three_tokens(void)
{
    ...
    rc = ioh_response_add_vary(&resp, "Origin");
    TEST_ASSERT_EQUAL_INT(0, rc);
