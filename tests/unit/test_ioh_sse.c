...
static ioh_sse_stream_t stream;

void setUp(void)
{
    int rc = ioh_sse_stream_init(&stream);
    TEST_ASSERT_EQUAL_INT(0, rc);
}

void tearDown(void)
{
    ioh_sse_stream_destroy(&stream);
}
...
{
    uint8_t buf[256];
    int rc = ioh_sse_format_headers(buf, sizeof(buf));
    ...
    /* buffer too small */
    int rc2 = ioh_sse_format_headers(buf, 10);
...
void test_sse_format_data_only(void)
{
    ...
    int rc = ioh_sse_format_event(&stream, &event);
    TEST_ASSERT_EQUAL_INT(0, rc);

    const uint8_t *out;
    size_t len;
    ioh_sse_stream_flush(&stream, &out, &len);
...
{
    int rc = ioh_sse_format_comment(&stream, "ping");
    TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_sse_stream_flush(void)
{
    ...
    int rc = ioh_sse_format_event(&stream, &event1);
    TEST_ASSERT_EQUAL_INT(0, rc);
    ...
    rc = ioh_sse_format_event(&stream, &event2);
    TEST_ASSERT_EQUAL_INT(0, rc);
