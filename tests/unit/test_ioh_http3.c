...
{
    uint8_t data[] = "test";
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_http3_on_stream_data(nullptr, 0, data, 4, false));
}

void test_http3_on_stream_open_null(void)
{
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_http3_on_stream_open(nullptr, 0));
}

void test_http3_submit_response_null(void)
{
    ioh_response_t resp = {0};
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_http3_submit_response(nullptr, 0, &resp));
}
