...
static int on_request_cb(ioh_ctx_t *c, int32_t stream_id, void *user_data)
{
...
static int pump(ioh_http2_session_t *server, test_buf_t *client_out, test_buf_t *server_out)
{
    ssize_t consumed = ioh_http2_on_recv(server, client_out->data, client_out->len);
    if (consumed < 0) {
        return (int)consumed;
    }
    client_out->len = 0;
    ...
    const uint8_t *data;
    size_t len;
    int rv = ioh_http2_flush(server, &data, &len);
    if (rv != 0) {
        return rv;
    }
...
{
    ioh_http2_session_t *session = ioh_http2_session_create(nullptr, nullptr, nullptr);
    TEST_ASSERT_NOT_NULL(session);

    /* Fresh session should want to write (SETTINGS) */
    TEST_ASSERT_TRUE(ioh_http2_want_write(session));
    TEST_ASSERT_FALSE(ioh_http2_goaway_sent(session));

    ioh_http2_session_destroy(session);

    /* Destroying nullptr should be safe */
    ioh_http2_session_destroy(nullptr);
}
...
void test_http2_connection_preface(void)
{
    ...
    ioh_http2_session_t *server = ioh_http2_session_create(nullptr, nullptr, nullptr);
    ...
    ioh_http2_session_destroy(server);
}
...
void test_http2_settings_ack(void)
{
    ...
    TEST_ASSERT_TRUE(ioh_http2_want_read(server));
...
void test_http2_simple_get(void)
{
    ...
    ioh_http2_session_t *server = ioh_http2_session_create(nullptr, on_request_cb, &ctx);
    nghttp2_session *client = make_client(&client_out);
    ...
    TEST_ASSERT_EQUAL_INT(IOH_METHOD_GET, ctx.last_req.method);
...
void test_http2_post_with_body(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(1, ctx.request_count);
    TEST_ASSERT_EQUAL_INT(IOH_METHOD_POST, ctx.last_req.method);
...
void test_http2_goaway_two_phase(void)
{
    ...
    TEST_ASSERT_FALSE(ioh_http2_goaway_sent(server));
    int rv = ioh_http2_goaway(server);
    TEST_ASSERT_EQUAL_INT(0, rv);
    TEST_ASSERT_TRUE(ioh_http2_goaway_sent(server));

    /* Flush to get the GOAWAY frame */
    server_out.len = 0;
    const uint8_t *data;
    size_t len;
    TEST_ASSERT_EQUAL_INT(0, ioh_http2_flush(server, &data, &len));
...
void test_http2_goaway_drains(void)
{
    ...
    /* Send GOAWAY */
    TEST_ASSERT_EQUAL_INT(0, ioh_http2_goaway(server));
...
void test_http2_max_concurrent_streams(void)
{
    ...
    ioh_http2_session_t *server = ioh_http2_session_create(&cfg, nullptr, nullptr);
    nghttp2_session *client = make_client(&client_out);
 ...
 * then ioh_response_destroy() freed resp->body, and nghttp2 later called
...
void test_http2_body_lifetime_after_response_destroy(void)
{
    ...
    /* Exchange connection preface */
    {
        ...
        /* Feed to server */
        ssize_t consumed = ioh_http2_on_recv(server, buf, total);
        TEST_ASSERT_GREATER_THAN(0, consumed);

        /* Flush server (SETTINGS + SETTINGS ACK) */
        const uint8_t *out;
        size_t out_len;
        TEST_ASSERT_EQUAL_INT(0, ioh_http2_flush(server, &out, &out_len));
        ...
        if (total > 0) {
            (void)ioh_http2_on_recv(server, buf, total);
            (void)ioh_http2_flush(server, &out, &out_len);
            if (out_len > 0) {
                nghttp2_session_mem_recv2(client, out, out_len);
            }
        }
     ...
     *   submit_response() → saves body pointer → ioh_response_destroy() → frees body
     * Without the fix, resp_data_read_cb would read freed memory. */
    ssize_t consumed = ioh_http2_on_recv(server, req_buf, req_total);
    TEST_ASSERT_GREATER_THAN(0, consumed);
    TEST_ASSERT_EQUAL_INT(1, ctx.request_count);
    ...
    size_t resp_out_len;
    TEST_ASSERT_EQUAL_INT(0, ioh_http2_flush(server, &resp_out, &resp_out_len));
