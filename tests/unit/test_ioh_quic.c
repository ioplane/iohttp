...
void test_quic_conn_create_null_cfg(void)
{
    ...
    TEST_ASSERT_NULL(ioh_quic_conn_create(nullptr, &cbs, dcid, 8, scid, 8, (struct sockaddr *)&local,
...
void test_quic_on_recv_null(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(-EINVAL, ioh_quic_on_recv(nullptr, buf, 10, (struct sockaddr *)&remote));
}
...
static int on_stream_data(ioh_quic_conn_t *conn, int64_t stream_id, const uint8_t *data, size_t len,
