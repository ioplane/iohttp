...
void test_http1_parse_get(void)
{
    ...
    int rc = ioh_http1_parse_request((const uint8_t *)raw, strlen(raw), &req);
...
void test_http1_parse_post_body(void)
{
    ...
    int rc = ioh_http1_parse_request((const uint8_t *)raw, strlen(raw), &req);
    TEST_ASSERT_GREATER_THAN(0, rc);
    TEST_ASSERT_EQUAL_UINT8(IOH_METHOD_POST, req.method);
...
void test_http1_parse_headers(void)
{
    ...
    const char *host = ioh_request_header(&req, "Host");
    ...
    const char *custom = ioh_request_header(&req, "X-Custom");
...
{
    /* Build a request with URI > IOH_HTTP1_MAX_URI_SIZE (4096) */
    char raw[8192 + 128];
    memcpy(raw, "GET /", 5);
    memset(raw + 5, 'A', 4200);
    ...
    ioh_request_t req;
    int rc = ioh_http1_parse_request((const uint8_t *)raw, 5 + 4200 + strlen(suffix), &req);
    TEST_ASSERT_EQUAL_INT(-E2BIG, rc);
}

void test_http1_parse_oversized_headers(void)
{
    /* Build a request with more headers than IOH_HTTP1_MAX_HEADERS (64) */
    char raw[32768];
    int pos = 0;
    ...
    ioh_request_t req;
    int rc = ioh_http1_parse_request((const uint8_t *)raw, (size_t)pos, &req);
...
{
    ioh_chunked_decoder_t dec;
    ioh_http1_chunked_init(&dec);

    char data[] = "4\r\nWiki\r\n0\r\n\r\n";
    size_t len = strlen(data);
    uint8_t *buf = (uint8_t *)data;

    int rc = ioh_http1_chunked_decode(&dec, buf, &len);
...
void test_http1_serialize_200(void)
{
    ...
    rc = ioh_response_set_body(&resp, (const uint8_t *)body, strlen(body));
    TEST_ASSERT_EQUAL_INT(0, rc);

    uint8_t buf[4096];
    rc = ioh_http1_serialize_response(&resp, buf, sizeof(buf));
    TEST_ASSERT_GREATER_THAN(0, rc);
...
void test_http1_serialize_headers(void)
{
    ...
    resp.status = 404;
    rc = ioh_response_set_header(&resp, "X-Custom", "value123");
    TEST_ASSERT_EQUAL_INT(0, rc);
    rc = ioh_response_set_header(&resp, "Content-Type", "text/plain");
    TEST_ASSERT_EQUAL_INT(0, rc);
