...
{
    ioh_encoding_t enc = ioh_compress_negotiate("gzip");
    TEST_ASSERT_EQUAL(IOH_ENCODING_GZIP, enc);
}

void test_compress_negotiate_br(void)
{
    ioh_encoding_t enc = ioh_compress_negotiate("br");
    TEST_ASSERT_EQUAL(IOH_ENCODING_BROTLI, enc);
}

void test_compress_negotiate_both(void)
{
    ioh_encoding_t enc = ioh_compress_negotiate("gzip, br");
    TEST_ASSERT_EQUAL(IOH_ENCODING_BROTLI, enc);
}

void test_compress_negotiate_none(void)
{
    ioh_encoding_t enc = ioh_compress_negotiate("identity");
    TEST_ASSERT_EQUAL(IOH_ENCODING_NONE, enc);
}

void test_compress_negotiate_qvalue(void)
{
    ioh_encoding_t enc = ioh_compress_negotiate("gzip;q=0.5, br;q=1.0");
    TEST_ASSERT_EQUAL(IOH_ENCODING_BROTLI, enc);
}
...
{
    char out[512];
    bool found = ioh_compress_precompressed(tmp_file, IOH_ENCODING_GZIP, out, sizeof(out));
...
{
    char out[512];
    bool found = ioh_compress_precompressed(tmp_file, IOH_ENCODING_BROTLI, out, sizeof(out));
    TEST_ASSERT_TRUE(found);
...
{
    ioh_compress_config_t cfg;
    ioh_compress_config_init(&cfg);
    ...
    rc = ioh_response_set_body(&resp, (const uint8_t *)small, strlen(small));
    TEST_ASSERT_EQUAL(0, rc);

    rc = ioh_compress_response(&cfg, &req, &resp);
    TEST_ASSERT_EQUAL(0, rc);
