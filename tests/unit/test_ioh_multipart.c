 ...
 * @brief Unit tests for ioh_multipart RFC 7578 parser.
 */
...
void test_multipart_single_field(void)
{
    ...
    ioh_multipart_config_t cfg;
    ioh_multipart_config_init(&cfg);

    ioh_multipart_part_t parts[8];
    uint32_t count = 8;

    int rc = ioh_multipart_parse((const uint8_t *)body, strlen(body), BOUNDARY, strlen(BOUNDARY),
                                &cfg, parts, &count);
...
void test_multipart_empty_body(void)
{
    ...
    int rc =
        ioh_multipart_parse((const uint8_t *)"", 0, BOUNDARY, strlen(BOUNDARY), &cfg, parts, &count);
    TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_multipart_missing_boundary(void)
{
    ...
    int rc = ioh_multipart_boundary("multipart/form-data", &bptr, &boundary_len);
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);

    /* not multipart at all */
    rc = ioh_multipart_boundary("application/json", &bptr, &boundary_len);
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);

    /* nullptr input */
    rc = ioh_multipart_boundary(nullptr, &bptr, &boundary_len);
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);

    /* valid boundary extraction */
    rc = ioh_multipart_boundary("multipart/form-data; boundary=----TestBoundary", &bptr,
                               &boundary_len);
    TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_multipart_oversized_part(void)
{
    ...
    int rc = ioh_multipart_parse(buf, off, BOUNDARY, strlen(BOUNDARY), &cfg, parts, &count);
...
void test_multipart_malformed(void)
{
    ...
    int rc = ioh_multipart_parse((const uint8_t *)garbage, strlen(garbage), BOUNDARY,
    ...
    const char *no_disposition = "------TestBoundary\r\n"
                                 ...
                                 "data\r\n"
                                 "------TestBoundary--\r\n";
    rc = ioh_multipart_parse((const uint8_t *)no_disposition, strlen(no_disposition), BOUNDARY,
                            strlen(BOUNDARY), &cfg, parts, &count);
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);
    ...
    rc = ioh_multipart_parse((const uint8_t *)truncated, strlen(truncated), BOUNDARY,
                            strlen(BOUNDARY), &cfg, parts, &count);
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);
}
