 ...
 * @brief Unit tests for io_buffer pool management.
 */
...
{
    io_bufpool_config_t cfg;
    io_bufpool_config_init(&cfg);
...
void test_bufpool_config_validate_valid(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, io_bufpool_config_validate(&cfg));
...
void test_bufpool_config_validate_invalid(void)
{
    ...
    cfg.ring_size = 100;
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_bufpool_config_validate(&cfg));
    ...
    /* nullptr config */
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_bufpool_config_validate(nullptr));
}
...
void test_bufpool_create_destroy(void)
{
    ...
    io_bufpool_t *pool = io_bufpool_create(&cfg);
    TEST_ASSERT_NOT_NULL(pool);

    TEST_ASSERT_EQUAL_UINT32(128, io_bufpool_ring_size(pool));
    TEST_ASSERT_EQUAL_UINT32(4096, io_bufpool_buf_size(pool));
    TEST_ASSERT_EQUAL_UINT32(0, io_bufpool_reg_buf_count(pool));
    TEST_ASSERT_EQUAL_UINT32(0, io_bufpool_reg_file_count(pool));

    /* Destroy without registration — no ring needed */
    io_bufpool_destroy(pool, nullptr);

    /* Destroying nullptr should not crash */
    io_bufpool_destroy(nullptr, nullptr);
}
...
void test_bufpool_get_buf_valid_id(void)
{
    ...
    uint8_t *b0 = io_bufpool_get_buf(pool, 0);
    TEST_ASSERT_NOT_NULL(b0);

    uint8_t *b1 = io_bufpool_get_buf(pool, 1);
    ...
    /* Last valid buffer */
    uint8_t *b15 = io_bufpool_get_buf(pool, 15);
...
void test_bufpool_get_buf_invalid_id(void)
{
    ...
    /* Out of range */
    TEST_ASSERT_NULL(io_bufpool_get_buf(pool, 16));
    TEST_ASSERT_NULL(io_bufpool_get_buf(pool, 1000));

    /* nullptr pool */
    TEST_ASSERT_NULL(io_bufpool_get_buf(nullptr, 0));
...
void test_bufpool_register_ring(void)
{
    ...
    int ret = io_bufpool_register_ring(pool, &test_ring);
    TEST_ASSERT_EQUAL_INT(0, ret);

    /* Get a buffer pointer */
    uint8_t *buf = io_bufpool_get_buf(pool, 0);
    TEST_ASSERT_NOT_NULL(buf);

    /* Write something to it and return */
    memset(buf, 0xAB, 64);
    io_bufpool_return_buf(pool, 0);

    /* Double registration should fail */
    ret = io_bufpool_register_ring(pool, &test_ring);
    TEST_ASSERT_EQUAL_INT(-EALREADY, ret);

    io_bufpool_destroy(pool, &test_ring);
}
...
void test_bufpool_register_bufs(void)
{
    ...
    int ret = io_bufpool_register_bufs(pool, &test_ring);
    TEST_ASSERT_EQUAL_INT(0, ret);

    /* Access registered buffers */
    uint8_t *rb0 = io_bufpool_get_reg_buf(pool, 0);
    TEST_ASSERT_NOT_NULL(rb0);

    uint8_t *rb3 = io_bufpool_get_reg_buf(pool, 3);
    TEST_ASSERT_NOT_NULL(rb3);

    /* Out of range */
    TEST_ASSERT_NULL(io_bufpool_get_reg_buf(pool, 4));

    /* Buffers should be reg_buf_size apart */
    TEST_ASSERT_EQUAL_PTR(rb0 + 8192, io_bufpool_get_reg_buf(pool, 1));

    /* Double registration should fail */
    ret = io_bufpool_register_bufs(pool, &test_ring);
    TEST_ASSERT_EQUAL_INT(-EALREADY, ret);
...
void test_bufpool_register_files(void)
{
    ...
    int ret = io_bufpool_register_files(pool, &test_ring);
    TEST_ASSERT_EQUAL_INT(0, ret);

    TEST_ASSERT_EQUAL_UINT32(8, io_bufpool_reg_file_count(pool));

    /* Double registration should fail */
    ret = io_bufpool_register_files(pool, &test_ring);
    TEST_ASSERT_EQUAL_INT(-EALREADY, ret);
...
void test_bufpool_register_fd_slot(void)
{
    ...
    /* Register read end */
    int slot0 = io_bufpool_register_fd(pool, &test_ring, pipefd[0]);
    TEST_ASSERT_GREATER_OR_EQUAL(0, slot0);

    /* Register write end */
    int slot1 = io_bufpool_register_fd(pool, &test_ring, pipefd[1]);
    TEST_ASSERT_GREATER_OR_EQUAL(0, slot1);
    TEST_ASSERT_NOT_EQUAL(slot0, slot1);

    /* Unregister read end */
    io_bufpool_unregister_fd(pool, &test_ring, slot0);

    /* Re-register should reuse the freed slot */
    int slot2 = io_bufpool_register_fd(pool, &test_ring, pipefd[0]);
