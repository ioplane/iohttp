...
{
    ioh_radix_tree_t *tree = ioh_radix_create();
    TEST_ASSERT_NOT_NULL(tree);
    TEST_ASSERT_NOT_NULL(tree->root);
    ioh_radix_destroy(tree);

    /* nullptr is safe */
    ioh_radix_destroy(nullptr);
}
...
void test_radix_insert_static(void)
{
    ...
    int rc = ioh_radix_insert(tree, "/users/list", &handler_a, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);

    ioh_radix_match_t match;
    rc = ioh_radix_lookup(tree, "/users/list", strlen("/users/list"), &match);
    TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_radix_insert_param(void)
{
    ...
    int rc = ioh_radix_insert(tree, "/users/:id", &handler_a, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);

    ioh_radix_match_t match;
    rc = ioh_radix_lookup(tree, "/users/42", strlen("/users/42"), &match);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_EQUAL_PTR(&handler_a, match.handler);
...
void test_radix_insert_wildcard(void)
{
    ...
    int rc = ioh_radix_insert(tree, "/static/*path", &handler_a, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);

    ioh_radix_match_t match;
    rc = ioh_radix_lookup(tree, "/static/js/app.js", strlen("/static/js/app.js"), &match);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_EQUAL_PTR(&handler_a, match.handler);
    TEST_ASSERT_EQUAL_UINT32(1, match.param_count);
...
void test_radix_lookup_static(void)
{
    ...
    int rc = ioh_radix_insert(tree, "/api/health", &handler_a, &metadata_a, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);
    rc = ioh_radix_insert(tree, "/api/version", &handler_b, &metadata_b, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);

    ioh_radix_match_t match;
    rc = ioh_radix_lookup(tree, "/api/health", strlen("/api/health"), &match);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_EQUAL_PTR(&handler_a, match.handler);
    TEST_ASSERT_EQUAL_PTR(&metadata_a, match.metadata);

    rc = ioh_radix_lookup(tree, "/api/version", strlen("/api/version"), &match);
    TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_radix_priority_static_over_param(void)
{
    ...
    int rc = ioh_radix_insert(tree, "/users/:id", &handler_a, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);
    rc = ioh_radix_insert(tree, "/users/list", &handler_b, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_radix_priority_param_over_wildcard(void)
{
    ...
    int rc = ioh_radix_insert(tree, "/files/*path", &handler_a, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);
    rc = ioh_radix_insert(tree, "/files/:name", &handler_b, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);

    ioh_radix_match_t match;
    /* /files/readme should match param, not wildcard */
    rc = ioh_radix_lookup(tree, "/files/readme", strlen("/files/readme"), &match);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_EQUAL_PTR(&handler_b, match.handler);
    TEST_ASSERT_EQUAL_UINT32(1, match.param_count);
    ...
    rc = ioh_radix_lookup(tree, "/files/dir/file", strlen("/files/dir/file"), &match);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_EQUAL_PTR(&handler_a, match.handler);
...
void test_radix_conflict_detection(void)
{
    ...
    int rc = ioh_radix_insert(tree, "/:id", &handler_a, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);

    /* Different param name at same position -> -EEXIST */
    rc = ioh_radix_insert(tree, "/:name", &handler_b, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(-EEXIST, rc);

    /* Same param name is allowed (but same handler -> -EEXIST) */
    rc = ioh_radix_insert(tree, "/:id", &handler_c, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(-EEXIST, rc);
...
void test_radix_compressed_prefix(void)
{
    ...
    int rc = ioh_radix_insert(tree, "/api/users", &handler_a, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);
    rc = ioh_radix_insert(tree, "/api/posts", &handler_b, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);

    /* Both should be found */
    ioh_radix_match_t match;
    rc = ioh_radix_lookup(tree, "/api/users", strlen("/api/users"), &match);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_EQUAL_PTR(&handler_a, match.handler);

    rc = ioh_radix_lookup(tree, "/api/posts", strlen("/api/posts"), &match);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_EQUAL_PTR(&handler_b, match.handler);
...
void test_radix_no_match(void)
{
    ...
    int rc = ioh_radix_insert(tree, "/users", &handler_a, nullptr, nullptr);
    TEST_ASSERT_EQUAL_INT(0, rc);

    ioh_radix_match_t match;
    rc = ioh_radix_lookup(tree, "/unknown", strlen("/unknown"), &match);
