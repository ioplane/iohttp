 ...
 */
static int walk_node(walk_state_t *state, const io_radix_node_t *node)
{
    if (!node) {
        return 0;
    }
...
int io_router_walk(const io_router_t *r, io_route_walk_fn fn, void *ctx)
{
    if (!r || !fn) {
        return -EINVAL;
    }

    uint32_t method_count = io_router_method_count();

    for (uint32_t m = 0; m < method_count; m++) {
        io_radix_tree_t *tree = io_router_get_tree(r, (io_method_t)m);
        if (!tree || !tree->root) {
            continue;
        }
...
static void count_node(const io_radix_node_t *node, uint32_t *count)
{
    if (!node) {
        return;
    }
