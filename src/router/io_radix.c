...
static io_radix_node_t *io_radix_node_create(const char *prefix, size_t prefix_len,
                                              io_node_type_t type)
{
    io_radix_node_t *node = calloc(1, sizeof(*node));
    if (!node) {
        return nullptr;
    }
...
static void io_radix_node_destroy(io_radix_node_t *node)
{
    if (!node) {
        return;
    }
    ...
    for (uint32_t i = 0; i < node->child_count; i++) {
        io_radix_node_destroy(node->children[i]);
    }
 ...
 */
static int io_radix_node_add_child(io_radix_node_t *parent, io_radix_node_t *child)
{
    if (parent->child_count >= parent->child_capacity) {
        uint32_t new_cap = parent->child_capacity == 0
                               ? RADIX_INITIAL_CAPACITY
                               : parent->child_capacity * 2;
        io_radix_node_t **new_children =
    ...
    while (pos < parent->child_count) {
        io_radix_node_t *existing = parent->children[pos];
        if (child->type < existing->type) {
            break;
        }
        if (child->type == existing->type && child->type == IO_NODE_STATIC &&
...
static void io_radix_increment_priority(io_radix_node_t *node)
{
    node->priority++;
}
 ...
 */
static io_radix_node_t *split_static_node(io_radix_node_t *parent, uint32_t child_idx,
                                           size_t split_pos)
{
    io_radix_node_t *existing = parent->children[child_idx];
    size_t existing_len = strlen(existing->prefix);

    /* Create intermediate node with the common prefix */
    io_radix_node_t *intermediate =
        io_radix_node_create(existing->prefix, split_pos, IO_NODE_STATIC);
    if (!intermediate) {
        return nullptr;
    }
    ...
    if (!remainder) {
        io_radix_node_destroy(intermediate);
        return nullptr;
    }
    ...
    int rc = io_radix_node_add_child(intermediate, existing);
    if (rc < 0) {
                                  ...
                                  size_t pattern_len, void *handler, void *metadata)
{
    ...
    for (uint32_t i = 0; i < node->child_count; i++) {
        ...
        if (cplen == child_prefix_len && cplen == seg_len) {
            ...
            return insert_recursive(child, pattern, next_pos, pattern_len,
                                    handler, metadata);
        }

        if (cplen < child_prefix_len) {
            /* Split existing node at cplen */
            io_radix_node_t *intermediate = split_static_node(node, i, cplen);
            if (!intermediate) {
                return -ENOMEM;
            }
            ...
            if (cplen == seg_len) {
                ...
                return insert_recursive(intermediate, pattern, next_pos,
                                        pattern_len, handler, metadata);
            }

            /* Create new child for remaining segment */
            io_radix_node_t *new_child =
                io_radix_node_create(seg + cplen, seg_len - cplen, IO_NODE_STATIC);
            if (!new_child) {
                return -ENOMEM;
            }

            int rc = io_radix_node_add_child(intermediate, new_child);
            if (rc < 0) {
                io_radix_node_destroy(new_child);
                return rc;
            }

            return insert_recursive(new_child, pattern, next_pos, pattern_len,
                                    handler, metadata);
        }
        ...
        return insert_static_segment(child, rest, rest_len, pattern, next_pos,
                                     pattern_len, handler, metadata);
    }

    /* No matching static child found; create new one */
    io_radix_node_t *new_child = io_radix_node_create(seg, seg_len, IO_NODE_STATIC);
    if (!new_child) {
        return -ENOMEM;
    }

    int rc = io_radix_node_add_child(node, new_child);
    if (rc < 0) {
        io_radix_node_destroy(new_child);
        return rc;
    }
                             ...
                             size_t pattern_len, void *handler, void *metadata)
{
    ...
    if (seg[0] == '*') {
        ...
        for (uint32_t i = 0; i < node->child_count; i++) {
            if (node->children[i]->type == IO_NODE_WILDCARD) {
                if (strncmp(node->children[i]->param_name, param_name,
                            param_name_len) != 0 ||
        ...
        io_radix_node_t *wc = io_radix_node_create("", 0, IO_NODE_WILDCARD);
        if (!wc) {
            return -ENOMEM;
        }
        wc->param_name = strndup(param_name, param_name_len);
        if (!wc->param_name) {
            io_radix_node_destroy(wc);
            return -ENOMEM;
        }
        wc->handler = handler;
    ...
    if (seg[0] == ':') {
        ...
        for (uint32_t i = 0; i < node->child_count; i++) {
            if (node->children[i]->type == IO_NODE_PARAM) {
                if (strncmp(node->children[i]->param_name, param_name,
                            param_name_len) != 0 ||
                    node->children[i]->param_name[param_name_len] != '\0') {
                    return -EEXIST;
                ...
                io_radix_increment_priority(node);
                return insert_recursive(node->children[i], pattern, next_pos,
                                        pattern_len, handler, metadata);
            }
        }

        io_radix_node_t *param = io_radix_node_create("", 0, IO_NODE_PARAM);
        if (!param) {
            return -ENOMEM;
        }
        param->param_name = strndup(param_name, param_name_len);
        if (!param->param_name) {
            io_radix_node_destroy(param);
            return -ENOMEM;
        }

        int rc = io_radix_node_add_child(node, param);
        if (rc < 0) {
            io_radix_node_destroy(param);
            return rc;
        }
        ...
        io_radix_increment_priority(node);
        return insert_recursive(param, pattern, next_pos, pattern_len,
                                handler, metadata);
    }

    /* Static segment */
    io_radix_increment_priority(node);
    return insert_static_segment(node, seg, seg_len, pattern, next_pos,
                                 pattern_len, handler, metadata);
}

/* ---- Public API ---- */

io_radix_tree_t *io_radix_create(void)
{
    io_radix_tree_t *tree = calloc(1, sizeof(*tree));
    if (!tree) {
        return nullptr;
    }

    tree->root = io_radix_node_create("", 0, IO_NODE_STATIC);
    if (!tree->root) {
        free(tree);
        return nullptr;
    }
...
void io_radix_destroy(io_radix_tree_t *tree)
{
    ...
    io_radix_node_destroy(tree->root);
    free(tree);
}

int io_radix_insert(io_radix_tree_t *tree, const char *pattern,
...
static int lookup_recursive(const io_radix_node_t *node, const char *path,
                             size_t pos, size_t path_len, io_radix_match_t *match);

/**
 * Try to match against static children, handling prefix compression.
 */
static int lookup_static(const io_radix_node_t *node, const char *seg,
                          size_t seg_len, const char *path, size_t next_pos,
                          size_t path_len, io_radix_match_t *match)
{
    for (uint32_t i = 0; i < node->child_count; i++) {
        const io_radix_node_t *child = node->children[i];
        if (child->type != IO_NODE_STATIC) {
            continue;
        }
        ...
        if (prefix_len == seg_len) {
            ...
            int rc = lookup_static(child, rest, rest_len, path, next_pos,
                                   path_len, match);
            if (rc == 0) {
                return 0;
            }
                             ...
                             size_t pos, size_t path_len, io_radix_match_t *match)
{
    ...
    int rc = lookup_static(node, path + seg_start, seg_len, path, seg_end,
                           path_len, match);
    if (rc == 0) {
        return 0;
    }
    ...
    for (uint32_t i = 0; i < node->child_count; i++) {
        ...
        /* Save param */
        if (match->param_count >= IO_MAX_PATH_PARAMS) {
            return -ENOENT;
        }
        uint32_t saved_count = match->param_count;
        io_param_t *p = &match->params[match->param_count++];
...
int io_radix_lookup(const io_radix_tree_t *tree, const char *path,
                    size_t path_len, io_radix_match_t *match)
{
    if (!tree || !path || !match) {
        return -EINVAL;
