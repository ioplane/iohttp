...
static const ioh_mime_entry_t mime_table[] = {
...
int ioh_etag_generate(uint64_t mtime, uint64_t size, char *buf, size_t buf_size)
{
...
int ioh_static_serve(const ioh_static_config_t *cfg, const ioh_request_t *req, ioh_response_t *resp)
{
    ...
    int etag_len = 0;
    if (cfg->etag) {
        etag_len = ioh_etag_generate(mtime, file_size, etag_buf, sizeof(etag_buf));
        if (etag_len < 0) {
            etag_len = 0;
        }
    }
    ...
    if (cfg->etag && etag_len > 0) {
        const char *inm = ioh_request_header(req, "If-None-Match");
        if (inm != nullptr) {
            size_t inm_len = strnlen(inm, IOH_MAX_HEADER_SIZE);
            if (inm_len == (size_t)etag_len && memcmp(inm, etag_buf, (size_t)etag_len) == 0) {
                resp->status = 304;
                (void)ioh_response_set_header(resp, "ETag", etag_buf);
                return 0;
            }
        }
    }
    ...
    const char *ims = ioh_request_header(req, "If-Modified-Since");
    if (ims != nullptr) {
        struct tm tm_parsed;
    ...
    const char *range_hdr = ioh_request_header(req, "Range");
    ...
    /* set response */
    const char *mime = ioh_mime_type(req->path, req->path_len);
    ...
    if (is_range) {
        ...
        (void)ioh_response_set_header(resp, "Content-Range", content_range);
    } else {
        resp->status = 200;
    }

    (void)ioh_response_set_header(resp, "Content-Type", mime);

    char len_str[32];
    snprintf(len_str, sizeof(len_str), "%lu", (unsigned long)bytes_read);
    (void)ioh_response_set_header(resp, "Content-Length", len_str);

    (void)ioh_response_set_header(resp, "Accept-Ranges", "bytes");
    ...
    if (gmtime_r((time_t *)&st.st_mtime, &tm_mtime) != nullptr) {
        if (strftime(lm_buf, sizeof(lm_buf), "%a, %d %b %Y %H:%M:%S GMT", &tm_mtime) > 0) {
            (void)ioh_response_set_header(resp, "Last-Modified", lm_buf);
        }
    }
    ...
    (void)ioh_response_set_header(resp, "Cache-Control", cc_buf);

    (void)ioh_response_set_body(resp, buf, bytes_read);
    free(buf);
