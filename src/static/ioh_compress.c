...
ioh_encoding_t ioh_compress_negotiate(const char *accept_encoding)
{
    ...
    if (br_q > 0.0 && br_q >= gzip_q) {
        return IOH_ENCODING_BROTLI;
    }
    if (gzip_q > 0.0) {
        return IOH_ENCODING_GZIP;
    }
                               ...
                               size_t out_size)
{
    ...
    switch (encoding) {
    case IOH_ENCODING_GZIP:
        suffix = ".gz";
        break;
    case IOH_ENCODING_BROTLI:
        suffix = ".br";
        break;
    default:
        return false;
...
int ioh_compress_response(const ioh_compress_config_t *cfg, const ioh_request_t *req,
                         ioh_response_t *resp)
{
    ...
    const char *ae = ioh_request_header(req, "Accept-Encoding");
    ioh_encoding_t enc = ioh_compress_negotiate(ae);

    /* always set Vary so caches know about encoding negotiation */
    int rc = ioh_response_add_vary(resp, "Accept-Encoding");
    if (rc < 0) {
        return rc;
    }
    ...
    if (enc == IOH_ENCODING_GZIP && cfg->enable_gzip) {
#ifdef IOHTTP_HAVE_ZLIB
        ...
        rc = ioh_response_set_header(resp, "Content-Encoding", "gzip");
#else
        return 0; /* no zlib — skip silently */
#endif
    } else if (enc == IOH_ENCODING_BROTLI && cfg->enable_brotli) {
#ifdef IOHTTP_HAVE_BROTLI
        ...
        rc = ioh_response_set_header(resp, "Content-Encoding", "br");
#else
    ...
    rc = ioh_response_set_body(resp, compressed, compressed_len);
    free(compressed);
    return rc;
}
