 ...
 */
static bool is_api_path(const ioh_spa_config_t *cfg, const char *path, size_t path_len)
{
    for (uint32_t i = 0; i < cfg->api_prefix_count; i++) {
        size_t prefix_len = strnlen(cfg->api_prefixes[i], IOH_MAX_URI_SIZE);
...
int ioh_spa_serve(const ioh_spa_config_t *cfg, const ioh_request_t *req, ioh_response_t *resp)
{
    ...
    if (file_exists) {
        /* 3. file exists — serve via ioh_static_serve */
        ioh_static_config_t static_cfg;
        ioh_static_config_init(&static_cfg);
        static_cfg.root_dir = cfg->root_dir;
        static_cfg.max_age_default = cfg->max_age_default;

        int rc = ioh_static_serve(&static_cfg, req, resp);
        if (rc != 0) {
            return rc;
        }

        /* override Cache-Control for hashed assets */
        if (ioh_spa_is_hashed_asset(req->path, req->path_len)) {
            char cc_buf[80];
    ...
    ioh_request_t index_req;
    ioh_request_init(&index_req);
    ...
    int rc = ioh_static_serve(&static_cfg, &index_req, resp);
    if (rc != 0) {
        return rc;
    }

    /* set no-cache for SPA fallback pages */
    (void)ioh_response_set_header(resp, "Cache-Control", "no-cache");
