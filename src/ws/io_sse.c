...
static int sse_ensure_capacity(io_sse_stream_t *stream, size_t additional)
{
...
static int sse_append(io_sse_stream_t *stream, const char *data, size_t len)
{
...
static int sse_append_str(io_sse_stream_t *stream, const char *str)
{
...
int io_sse_stream_init(io_sse_stream_t *stream)
{
    if (stream == nullptr) {
        return -EINVAL;
    }
    ...
    stream->buf = malloc(IO_SSE_INITIAL_BUF);
    if (stream->buf == nullptr) {
        return -ENOMEM;
    }
    stream->buf_capacity = IO_SSE_INITIAL_BUF;

    return 0;
}

void io_sse_stream_destroy(io_sse_stream_t *stream)
{
    if (stream == nullptr) {
        return;
    }
...
int io_sse_format_headers(uint8_t *buf, size_t buf_size)
{
    if (buf == nullptr) {
        return -EINVAL;
    }

    size_t len = sizeof(IO_SSE_HEADERS) - 1;
    if (buf_size < len) {
        return -ENOSPC;
    }

    memcpy(buf, IO_SSE_HEADERS, len);
    return (int)len;
}

int io_sse_format_event(io_sse_stream_t *stream, const io_sse_event_t *event)
{
...
int io_sse_format_comment(io_sse_stream_t *stream, const char *comment)
{
