...
constexpr size_t IOH_COOKIE_NAME_MAX = 256;

int ioh_cookie_validate_name(const char *name)
{
    if (name == nullptr) {
        return -EINVAL;
    }

    size_t len = strnlen(name, IOH_COOKIE_NAME_MAX + 1);
    if (len == 0 || len > IOH_COOKIE_NAME_MAX) {
        return -EINVAL;
    }
...
int ioh_cookie_serialize(const ioh_cookie_t *cookie, char *buf, size_t buf_size)
{
    if (cookie == nullptr || buf == nullptr || buf_size == 0) {
        return -EINVAL;
    }

    int rc = ioh_cookie_validate_name(cookie->name);
    if (rc != 0) {
        return rc;
    }
    ...
    if (cookie->same_site == IOH_SAME_SITE_NONE) {
        secure = true;
    }
