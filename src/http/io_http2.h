 ...
 *
 * No socket I/O — application feeds raw bytes via io_http2_on_recv(),
 * retrieves output via io_http2_flush(). Same pattern as WebSocket/SSE.
 */
...
typedef struct io_http2_session io_http2_session_t;
 ...
 */
[[nodiscard]] io_http2_session_t *io_http2_session_create(const io_http2_config_t *cfg,
                                                          io_http2_on_request_fn on_req,
                                                          void *user_data);
 ...
 */
[[nodiscard]] int io_http2_flush(io_http2_session_t *session, const uint8_t **out_data,
                                 size_t *out_len);
 ...
 */
[[nodiscard]] int io_http2_goaway(io_http2_session_t *session);
