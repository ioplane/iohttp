 ...
 */
static int parse_part_headers(const char *headers, size_t headers_len, io_multipart_part_t *part)
{
...
void io_multipart_config_init(io_multipart_config_t *cfg)
{
    if (!cfg) {
        return;
    }
    cfg->max_parts = IO_MULTIPART_DEFAULT_MAX_PARTS;
    cfg->max_part_size = IO_MULTIPART_DEFAULT_MAX_PART_SIZE;
    cfg->max_total_size = IO_MULTIPART_DEFAULT_MAX_TOTAL_SIZE;
}

int io_multipart_boundary(const char *content_type, const char **boundary, size_t *boundary_len)
{
...
int io_multipart_parse(const uint8_t *body, size_t body_len, const char *boundary,
                       ...
                       io_multipart_part_t *parts, uint32_t *part_count)
{
    ...
    *part_count = 0;
    ...
    while (remain > 0) {
        ...
        /* parse part */
        io_multipart_part_t *part = &parts[*part_count];
