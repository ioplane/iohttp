...
constexpr uint32_t IOH_RESPONSE_INITIAL_HEADERS = 16;
constexpr size_t IOH_RESPONSE_INITIAL_BODY = 1024;

int ioh_response_init(ioh_response_t *resp)
{
    if (resp == nullptr) {
        return -EINVAL;
    }
    ...
    resp->headers = calloc(IOH_RESPONSE_INITIAL_HEADERS, sizeof(*resp->headers));
    if (resp->headers == nullptr) {
        return -ENOMEM;
    }
    resp->header_capacity = IOH_RESPONSE_INITIAL_HEADERS;

    resp->body = malloc(IOH_RESPONSE_INITIAL_BODY);
    if (resp->body == nullptr) {
        free(resp->headers);
        resp->headers = nullptr;
        return -ENOMEM;
    }
    resp->body_capacity = IOH_RESPONSE_INITIAL_BODY;
...
int ioh_response_set_header(ioh_response_t *resp, const char *name, const char *value)
{
    ...
    size_t value_len = strnlen(value, IOH_MAX_HEADER_SIZE);
    ...
    if (resp->header_count >= resp->header_capacity) {
        ...
        ioh_header_t *tmp = realloc(resp->headers, new_cap * sizeof(*tmp));
        if (tmp == nullptr) {
            return -ENOMEM;
        }
        resp->headers = tmp;
    ...
    ioh_header_t *hdr = &resp->headers[resp->header_count];
    hdr->name = dup_name;
...
int ioh_response_add_header(ioh_response_t *resp, const char *name, const char *value)
{
    if (resp == nullptr || name == nullptr || value == nullptr) {
        return -EINVAL;
    }
...
int ioh_response_add_vary(ioh_response_t *resp, const char *token)
{
    if (resp == nullptr || token == nullptr) {
        return -EINVAL;
    }

    size_t token_len = strnlen(token, IOH_MAX_HEADER_SIZE);
    if (token_len == 0) {
        return -EINVAL;
    }
    ...
    return ioh_response_set_header(resp, "Vary", token);
}

int ioh_response_set_body(ioh_response_t *resp, const uint8_t *body, size_t len)
{
    if (resp == nullptr) {
        return -EINVAL;
    }
...
int ioh_respond(ioh_response_t *resp, uint16_t status, const char *content_type, const uint8_t *body,
               size_t body_len)
{
    if (resp == nullptr) {
        return -EINVAL;
    ...
    if (content_type != nullptr) {
        int rc = ioh_response_set_header(resp, "Content-Type", content_type);
        if (rc != 0) {
            return rc;
        }
    }
    ...
    if (body != nullptr && body_len > 0) {
        int rc = ioh_response_set_body(resp, body, body_len);
        if (rc != 0) {
            return rc;
        }
    }
...
int ioh_respond_json(ioh_response_t *resp, uint16_t status, const char *json)
{
    ...
    return ioh_respond(resp, status, "application/json", (const uint8_t *)json, len);
}
