 ...
 * via ioh_http2_on_recv(), retrieves output via ioh_http2_flush().
 */
...
typedef struct h2_resp_data {
    uint8_t *body; /* owned — transferred from ioh_response_t in submit_response() */
    size_t body_len;
    size_t offset;
} h2_resp_data_t;
...
struct ioh_http2_session {
    nghttp2_session *ng_session;
    ioh_http2_config_t config;
    ioh_http2_on_request_fn on_request;
    void *user_data;
...
static ioh_method_t parse_method(const char *method, size_t len)
{
    return ioh_method_parse(method, len);
}

static int output_buf_append(ioh_http2_session_t *session, const uint8_t *data, size_t len)
{
...
static int submit_response(ioh_http2_session_t *session, int32_t stream_id, ioh_response_t *resp)
{
    ...
    if (resp->body != nullptr && resp->body_len > 0) {
         ...
         * after ioh_response_destroy() frees resp. */
        rd->body = resp->body;
...
static int on_begin_headers_cb(nghttp2_session *session, const nghttp2_frame *frame,
                               void *user_data)
{
    ioh_http2_session_t *h2 = user_data;
                        ...
                        void *user_data)
{
    ...
    if (req->header_count < IOH_MAX_HEADERS) {
        char *name_copy = arena_copy(&sd->arena, (const char *)name, namelen);
        if (name_copy == nullptr) {
            return NGHTTP2_ERR_CALLBACK_FAILURE;
        }

        req->headers[req->header_count] = (ioh_header_t){
            .name = name_copy,
            .name_len = namelen,
            .value = val_copy,
            .value_len = valuelen,
...
static int on_frame_recv_cb(nghttp2_session *session, const nghttp2_frame *frame, void *user_data)
{
    ...
    if (h2->on_request != nullptr) {
        ...
        ioh_ctx_t ctx;
        rc = ioh_ctx_init(&ctx, &sd->request, &resp, nullptr);
        if (rc == 0) {
...
ioh_http2_session_t *ioh_http2_session_create(const ioh_http2_config_t *cfg,
                                            ioh_http2_on_request_fn on_req, void *user_data)
{
    ioh_http2_session_t *session = calloc(1, sizeof(*session));
    if (session == nullptr) {
        return nullptr;
    }
...
ssize_t ioh_http2_on_recv(ioh_http2_session_t *session, const uint8_t *data, size_t len)
{
...
int ioh_http2_flush(ioh_http2_session_t *session, const uint8_t **out_data, size_t *out_len)
{
...
int ioh_http2_goaway(ioh_http2_session_t *session)
{
    if (session == nullptr) {
        return -EINVAL;
    }
...
bool ioh_http2_want_read(const ioh_http2_session_t *session)
{
...
bool ioh_http2_want_write(const ioh_http2_session_t *session)
{
    if (session == nullptr || session->ng_session == nullptr) {
        return false;
    }
...
bool ioh_http2_goaway_sent(const ioh_http2_session_t *session)
{
    if (session == nullptr) {
        return false;
    }
...
bool ioh_http2_is_draining(const ioh_http2_session_t *session)
{
    if (session == nullptr) {
        return false;
    }
    return session->goaway_sent && !ioh_http2_want_read(session) && !ioh_http2_want_write(session);
}
