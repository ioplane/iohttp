                               ...
                               void *user_data, void *stream_user_data)
{
    ...
    ioh_quic_conn_t *qconn = user_data;
...
{
    ioh_quic_conn_t *qconn = conn_ref->user_data;
    return qconn->ngtcp2_conn;
}

/* ---- wolfSSL QUIC setup ---- */

static int setup_wolfssl(ioh_quic_conn_t *qconn, const ioh_quic_config_t *cfg)
{
...
int ioh_quic_config_validate(const ioh_quic_config_t *cfg)
{
    if (cfg == nullptr) {
        return -EINVAL;
    }
...
ioh_quic_conn_t *ioh_quic_conn_create(const ioh_quic_config_t *cfg, const ioh_quic_callbacks_t *cbs,
                                    ...
                                    const struct sockaddr *remote, void *user_data)
{
    ...
    if (ioh_quic_config_validate(cfg) != 0) {
        return nullptr;
    }

    ioh_quic_conn_t *qconn = calloc(1, sizeof(*qconn));
    if (qconn == nullptr) {
        return nullptr; //-V773
    }
...
int ioh_quic_on_recv(ioh_quic_conn_t *conn, const uint8_t *data, size_t len,
...
int ioh_quic_flush(ioh_quic_conn_t *conn, const uint8_t **out_data, size_t *out_len)
{
...
int ioh_quic_on_timeout(ioh_quic_conn_t *conn)
{
    if (conn == nullptr) {
        return -EINVAL;
    }
...
ssize_t ioh_quic_write_stream(ioh_quic_conn_t *conn, int64_t stream_id, const uint8_t *data,
                             size_t len, bool fin)
{
...
uint64_t ioh_quic_get_timeout(const ioh_quic_conn_t *conn)
{
...
bool ioh_quic_is_handshake_done(const ioh_quic_conn_t *conn)
{
    if (conn == nullptr || conn->ngtcp2_conn == nullptr) {
        return false;
    }
...
bool ioh_quic_is_closed(const ioh_quic_conn_t *conn)
{
    if (conn == nullptr) {
        return true;
    }
...
bool ioh_quic_want_write(const ioh_quic_conn_t *conn)
{
...
int ioh_quic_open_uni_stream(ioh_quic_conn_t *conn, int64_t *stream_id)
{
...
int ioh_quic_close(ioh_quic_conn_t *conn, uint64_t error_code)
{
    if (conn == nullptr) {
        return -EINVAL;
    }
