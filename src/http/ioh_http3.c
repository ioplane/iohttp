...
static h3_stream_data_t *stream_data_new(int64_t stream_id)
{
    ...
    ioh_request_init(&sd->request); //-V522
                            ...
                            void *stream_user_data)
{
    ...
    ioh_http3_session_t *h3 = conn_user_data;
    h3_stream_data_t *sd = stream_user_data;
    ...
    if (h3->on_request != nullptr) {
        ...
        if (rc == 0) {
            ...
            if (rc == 0 && !ctx.aborted) {
                (void)ioh_http3_submit_response(h3, stream_id, &resp);
            }
            ioh_ctx_destroy(&ctx);
        }
...
ioh_http3_session_t *ioh_http3_session_create(const ioh_http3_config_t *cfg, ioh_quic_conn_t *quic_conn,
                                            ioh_http3_on_request_fn on_req, void *user_data)
{
    if (quic_conn == nullptr) {
        return nullptr;
    }

    ioh_http3_session_t *session = calloc(1, sizeof(*session));
    if (session == nullptr) {
        return nullptr; //-V773
    }
    ...
    if (cfg != nullptr) {
        ...
        ioh_http3_config_init(&session->config);
    }
     ...
     * binding is deferred — call ioh_http3_bind_control_streams() after
     * handshake completes. */
    (void)ioh_http3_bind_control_streams(session);
...
int ioh_http3_bind_control_streams(ioh_http3_session_t *session)
{
    if (session == nullptr) {
        return -EINVAL;
    }
    ...
    if (ioh_quic_open_uni_stream(session->quic_conn, &ctrl_stream_id) != 0 ||
        ioh_quic_open_uni_stream(session->quic_conn, &qenc_stream_id) != 0 ||
        ioh_quic_open_uni_stream(session->quic_conn, &qdec_stream_id) != 0) {
        return -EAGAIN;
    }
...
int ioh_http3_on_stream_data(ioh_http3_session_t *session, int64_t stream_id, const uint8_t *data,
                            size_t len, bool fin)
{
    if (session == nullptr) {
        return -EINVAL;
...
int ioh_http3_on_stream_open(ioh_http3_session_t *session, int64_t stream_id)
{
    if (session == nullptr) {
        return -EINVAL;
    }
...
int ioh_http3_submit_response(ioh_http3_session_t *session, int64_t stream_id,
...
bool ioh_http3_is_shutdown(const ioh_http3_session_t *session)
{
    if (session == nullptr) {
        return true;
    }
...
int ioh_http3_shutdown(ioh_http3_session_t *session)
{
    if (session == nullptr) {
        return -EINVAL;
    }
