 ...
 */
static int decode_v1(const uint8_t *buf, size_t len, ioh_proxy_result_t *result)
{
    ...
    const uint8_t *crlf = find_crlf(buf, len, IOH_PROXY_V1_MAX_LEN);

    if (crlf == nullptr) {
        /* If we have IOH_PROXY_V1_MAX_LEN bytes and still no CRLF, malformed */
        if (len >= IOH_PROXY_V1_MAX_LEN) {
            return -EINVAL;
        }
        return -EAGAIN;
    }
    ...
    char line[IOH_PROXY_V1_MAX_LEN + 1];
 ...
 */
static int decode_v2(const uint8_t *buf, size_t len, ioh_proxy_result_t *result)
{
    if (len < IOH_PROXY_V2_MIN_LEN) {
        return -EAGAIN;
    }

    /* Verify signature */
    if (memcmp(buf, IOH_PROXY_V2_SIG, 12) != 0) {
        return -EINVAL;
    }
    ...
    size_t total = IOH_PROXY_V2_MIN_LEN + addr_len;
    ...
    const uint8_t *addrs = buf + IOH_PROXY_V2_MIN_LEN;
...
int ioh_proxy_decode(const uint8_t *buf, size_t len, ioh_proxy_result_t *result)
{
    ...
    if (len >= 12 && memcmp(buf, IOH_PROXY_V2_SIG, 12) == 0) {
