 ...
 */
static int tls_ioh_recv_cb(WOLFSSL *ssl, char *buf, int sz, void *ctx)
{
    (void)ssl;
    ioh_tls_conn_t *conn = ctx;
 ...
 */
static int tls_ioh_send_cb(WOLFSSL *ssl, char *buf, int sz, void *ctx)
{
    (void)ssl;
    ioh_tls_conn_t *conn = ctx;
...
int ioh_tls_config_validate(const ioh_tls_config_t *cfg)
{
    if (cfg == nullptr) {
        return -EINVAL;
    }
...
ioh_tls_ctx_t *ioh_tls_ctx_create(const ioh_tls_config_t *cfg)
{
    if (ioh_tls_config_validate(cfg) != 0) {
        return nullptr;
    }

    ensure_wolfssl_init();

    ioh_tls_ctx_t *ctx = calloc(1, sizeof(*ctx));
    if (ctx == nullptr) {
        return nullptr;
    }
    ...
    if (wolfSSL_CTX_SetMinVersion(ctx->wolf_ctx, WOLFSSL_TLSV1_2) != WOLFSSL_SUCCESS) {
        goto fail_ctx;
    }

    /* Load server certificate */
    if (wolfSSL_CTX_use_certificate_file(ctx->wolf_ctx, cfg->cert_file, WOLFSSL_FILETYPE_PEM) !=
        WOLFSSL_SUCCESS) {
        goto fail_ctx;
    }

    /* Load server private key */
    if (wolfSSL_CTX_use_PrivateKey_file(ctx->wolf_ctx, cfg->key_file, WOLFSSL_FILETYPE_PEM) !=
        WOLFSSL_SUCCESS) {
        goto fail_ctx;
    }
    ...
    if (cfg->ca_file != nullptr) {
        if (wolfSSL_CTX_load_verify_locations(ctx->wolf_ctx, cfg->ca_file, nullptr) !=
            WOLFSSL_SUCCESS) {
            goto fail_ctx;
        }
    }
    ...
    if (cfg->require_client_cert) {
        wolfSSL_CTX_set_verify(ctx->wolf_ctx,
                               WOLFSSL_VERIFY_PEER | WOLFSSL_VERIFY_FAIL_IF_NO_PEER_CERT, nullptr);
    }

    /* Session cache */
    if (cfg->session_cache_size == 0) {
        wolfSSL_CTX_set_session_cache_mode(ctx->wolf_ctx, WOLFSSL_SESS_CACHE_OFF);
    }

    /* Session tickets */
    if (!cfg->enable_session_tickets) {
        wolfSSL_CTX_set_options(ctx->wolf_ctx, WOLFSSL_OP_NO_TICKET);
    }
    ...
    wolfSSL_CTX_SetIORecv(ctx->wolf_ctx, tls_ioh_recv_cb);
    wolfSSL_CTX_SetIOSend(ctx->wolf_ctx, tls_ioh_send_cb);
...
ioh_tls_conn_t *ioh_tls_conn_create(ioh_tls_ctx_t *ctx, int fd)
{
    if (ctx == nullptr) {
        return nullptr;
    }

    ioh_tls_conn_t *conn = calloc(1, sizeof(*conn));
    if (conn == nullptr) {
        return nullptr;
    }

    conn->cipher_in_buf = malloc(IOH_TLS_CIPHER_BUF_SIZE);
    if (conn->cipher_in_buf == nullptr) {
        goto fail;
    }
    conn->cipher_in_cap = IOH_TLS_CIPHER_BUF_SIZE;

    conn->cipher_out_buf = malloc(IOH_TLS_CIPHER_BUF_SIZE);
    if (conn->cipher_out_buf == nullptr) {
        goto fail;
    }
    conn->cipher_out_cap = IOH_TLS_CIPHER_BUF_SIZE;
    ...
    if (ctx->alpn_copy != nullptr && ctx->alpn_copy_len > 0) {
        if (wolfSSL_UseALPN(conn->ssl, ctx->alpn_copy, ctx->alpn_copy_len,
...
int ioh_tls_feed_input(ioh_tls_conn_t *conn, const uint8_t *data, size_t len)
{
    ...
    if (conn->cipher_in_pos > 0 && conn->cipher_in_len > conn->cipher_in_pos) {
        ...
        memmove(conn->cipher_in_buf, conn->cipher_in_buf + conn->cipher_in_pos, remaining);
...
int ioh_tls_handshake(ioh_tls_conn_t *conn)
{
    if (conn == nullptr) {
        return -EINVAL;
    }
...
int ioh_tls_read(ioh_tls_conn_t *conn, uint8_t *buf, size_t len)
{
...
int ioh_tls_write(ioh_tls_conn_t *conn, const uint8_t *buf, size_t len)
{
    if (conn == nullptr || buf == nullptr) {
        return -EINVAL;
    }
...
int ioh_tls_get_output(ioh_tls_conn_t *conn, const uint8_t **data, size_t *len)
{
...
int ioh_tls_shutdown(ioh_tls_conn_t *conn)
{
    if (conn == nullptr) {
        return -EINVAL;
    }
