...
typedef struct {
    ioh_auth_verify_fn verify;
    void *ctx;
} ioh_auth_state_t;

static _Thread_local ioh_auth_state_t *tl_basic_state;
static _Thread_local ioh_auth_state_t *tl_bearer_state;
...
static int basic_middleware(ioh_ctx_t *c, ioh_handler_fn next)
{
    ioh_auth_state_t *st = tl_basic_state;
    const char *auth = ioh_request_header(c->req, "Authorization");

    if (auth == nullptr || strncmp(auth, "Basic ", 6) != 0) {
        c->resp->status = 401;
        (void)ioh_response_set_header(c->resp, "WWW-Authenticate", "Basic realm=\"iohttp\"");
        return 0;
    }
...
static int bearer_middleware(ioh_ctx_t *c, ioh_handler_fn next)
{
    ioh_auth_state_t *st = tl_bearer_state;
    const char *auth = ioh_request_header(c->req, "Authorization");

    if (auth == nullptr || strncmp(auth, "Bearer ", 7) != 0) {
        c->resp->status = 401;
        (void)ioh_response_set_header(c->resp, "WWW-Authenticate", "Bearer");
        return 0;
    }
...
ioh_middleware_fn ioh_auth_basic_create(ioh_auth_verify_fn verify, void *ctx)
{
    if (verify == nullptr) {
        return nullptr;
    }

    ioh_auth_state_t *st = malloc(sizeof(*st));
    if (st == nullptr) {
        return nullptr;
    }
    st->verify = verify;
...
ioh_middleware_fn ioh_auth_bearer_create(ioh_auth_verify_fn verify, void *ctx)
{
    if (verify == nullptr) {
        return nullptr;
    }
