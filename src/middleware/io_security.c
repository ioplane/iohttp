...
static int security_middleware(io_ctx_t *c, io_handler_fn next)
{
    int rc = next(c);
    if (rc != 0) {
        return rc;
    ...
    if (st->csp != nullptr) {
        rc = io_response_set_header(c->resp, "Content-Security-Policy", st->csp);
        if (rc != 0) {
            return rc;
        }
    }
    ...
    if (st->hsts) {
        ...
        rc = io_response_set_header(c->resp, "Strict-Transport-Security", buf);
        if (rc != 0) {
            return rc;
        }
    }
    ...
    if (st->frame_options != nullptr) {
        rc = io_response_set_header(c->resp, "X-Frame-Options", st->frame_options);
        if (rc != 0) {
            return rc;
        }
    }
    ...
    if (st->referrer_policy != nullptr) {
        rc = io_response_set_header(c->resp, "Referrer-Policy", st->referrer_policy);
        if (rc != 0) {
            return rc;
        }
    }
    ...
    if (st->nosniff) {
        rc = io_response_set_header(c->resp, "X-Content-Type-Options", "nosniff");
        if (rc != 0) {
            return rc;
        }
    }
