...
int ioh_ctx_init(ioh_ctx_t *c, ioh_request_t *req, ioh_response_t *resp, ioh_server_t *srv)
{
    ...
    c->arena.base = malloc(IOH_CTX_ARENA_DEFAULT);
    if (c->arena.base == nullptr) {
        return -ENOMEM;
    }
    c->arena.size = IOH_CTX_ARENA_DEFAULT;
    c->arena.used = 0;
...
int ioh_ctx_set(ioh_ctx_t *c, const char *key, void *value)
{
    return ioh_ctx_set_with_destructor(c, key, value, nullptr);
}

int ioh_ctx_set_with_destructor(ioh_ctx_t *c, const char *key, void *value, void (*dtor)(void *))
{
    ...
    /* append new */
    if (c->value_count >= IOH_CTX_MAX_VALUES) {
        return -ENOSPC;
    }
...
const char *ioh_ctx_param(const ioh_ctx_t *c, const char *name)
{
    ...
    return ioh_request_param(c->req, name);
}

int ioh_ctx_param_i64(const ioh_ctx_t *c, const char *name, int64_t *out)
{
    if (c == nullptr || c->req == nullptr) {
        return -EINVAL;
    }
    return ioh_request_param_i64(c->req, name, out);
}

int ioh_ctx_param_u64(const ioh_ctx_t *c, const char *name, uint64_t *out)
{
    if (c == nullptr || c->req == nullptr) {
        return -EINVAL;
    }
    return ioh_request_param_u64(c->req, name, out);
}
...
const char *ioh_ctx_query(const ioh_ctx_t *c, const char *name)
{
    ...
    return ioh_request_query_param(c->req, name);
}
...
const char *ioh_ctx_header(const ioh_ctx_t *c, const char *name)
{
    ...
    return ioh_request_header(c->req, name);
}
...
const char *ioh_ctx_cookie(const ioh_ctx_t *c, const char *name)
{
    ...
    return ioh_request_cookie(c->req, name);
}
...
int ioh_ctx_status(ioh_ctx_t *c, uint16_t status)
{
...
int ioh_ctx_set_header(ioh_ctx_t *c, const char *name, const char *value)
{
    if (c == nullptr || c->resp == nullptr) {
        return -EINVAL;
    }
    return ioh_response_set_header(c->resp, name, value);
}

int ioh_ctx_set_cookie(ioh_ctx_t *c, const ioh_cookie_t *cookie)
{
    ...
    char buf[512];
    int len = ioh_cookie_serialize(cookie, buf, sizeof(buf));
    if (len < 0) {
        return len;
    }

    return ioh_response_add_header(c->resp, "Set-Cookie", buf);
}

int ioh_ctx_json(ioh_ctx_t *c, uint16_t status, const char *json)
{
    if (c == nullptr || c->resp == nullptr) {
        return -EINVAL;
    }
    return ioh_respond_json(c->resp, status, json);
}

int ioh_ctx_text(ioh_ctx_t *c, uint16_t status, const char *text)
{
    if (c == nullptr || c->resp == nullptr || text == nullptr) {
        return -EINVAL;
    }
    size_t len = strnlen(text, 1024 * 1024);
    return ioh_respond(c->resp, status, "text/plain", (const uint8_t *)text, len);
}

int ioh_ctx_html(ioh_ctx_t *c, uint16_t status, const char *html)
{
    if (c == nullptr || c->resp == nullptr || html == nullptr) {
        return -EINVAL;
    }
    size_t len = strnlen(html, 1024 * 1024);
    return ioh_respond(c->resp, status, "text/html", (const uint8_t *)html, len);
}

int ioh_ctx_blob(ioh_ctx_t *c, uint16_t status, const char *content_type, const uint8_t *data,
                size_t len)
{
    if (c == nullptr || c->resp == nullptr) {
        return -EINVAL;
    }
    return ioh_respond(c->resp, status, content_type, data, len);
}

int ioh_ctx_no_content(ioh_ctx_t *c)
{
    if (c == nullptr || c->resp == nullptr) {
        return -EINVAL;
    }
...
int ioh_ctx_redirect(ioh_ctx_t *c, uint16_t status, const char *url)
{
    if (c == nullptr || c->resp == nullptr || url == nullptr) {
        return -EINVAL;
    }
    c->resp->status = status;
    return ioh_response_set_header(c->resp, "Location", url);
}

int ioh_ctx_error(ioh_ctx_t *c, uint16_t status, const char *message)
{
    ...
    return ioh_respond_json(c->resp, status, buf);
}
...
static int arena_grow(ioh_arena_t *a, size_t needed)
{
