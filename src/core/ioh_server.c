...
int ioh_server_config_validate(const ioh_server_config_t *cfg)
{
    if (cfg == nullptr) {
        return -EINVAL;
    }
...
ioh_server_t *ioh_server_create(const ioh_server_config_t *cfg)
{
    if (cfg == nullptr) {
        return nullptr;
    }
    if (ioh_server_config_validate(cfg) != 0) {
        return nullptr;
    }
    ...
    ioh_server_t *srv = calloc(1, sizeof(*srv));
    if (srv == nullptr) {
        return nullptr;
    }
    ...
    ioh_loop_config_t loop_cfg;
    ioh_loop_config_init(&loop_cfg);
    loop_cfg.queue_depth = cfg->queue_depth;

    srv->loop = ioh_loop_create(&loop_cfg);
    if (srv->loop == nullptr) {
        free(srv);
        return nullptr;
    }
    ...
    srv->pool = ioh_conn_pool_create(cfg->max_connections);
    if (srv->pool == nullptr) {
        ioh_loop_destroy(srv->loop);
        free(srv);
        return nullptr;
    }
...
void ioh_server_destroy(ioh_server_t *srv)
{
    ...
    ioh_conn_pool_destroy(srv->pool);
    ioh_loop_destroy(srv->loop);
    free(srv);
}
...
static int arm_multishot_accept(ioh_server_t *srv)
{
...
static int arm_send(ioh_server_t *srv, ioh_conn_t *conn, const uint8_t *data, size_t len)
{
    if (conn->send_active) {
        return -EBUSY;
    }
...
static void send_error_response(ioh_server_t *srv, ioh_conn_t *conn, uint16_t status, const char *msg)
{
    ioh_response_t resp;
    (void)ioh_response_init(&resp);
    resp.status = status;
    (void)ioh_response_set_body(&resp, (const uint8_t *)msg, strlen(msg));
...
static int dispatch_request(ioh_server_t *srv, ioh_conn_t *conn, ioh_request_t *req)
{
    ...
    if (srv->router != nullptr) {
        ioh_route_match_t m = ioh_router_dispatch(srv->router, req->method, req->path, req->path_len);
        ...
        if (m.status == IOH_MATCH_FOUND && m.handler != nullptr) {
            req->param_count = m.param_count;
            for (uint32_t i = 0; i < m.param_count && i < IOH_MAX_PATH_PARAMS; i++) {
                req->params[i] = m.params[i];
            }

            uint32_t global_count = 0;
            ioh_middleware_fn *global_mw = ioh_router_global_middleware(srv->router, &global_count);
            rc = ioh_chain_execute(&ctx, global_mw, global_count, nullptr, 0, m.handler);
...
int ioh_server_listen(ioh_server_t *srv)
{
    if (srv == nullptr) {
        return -EINVAL;
    }
    ...
    IOH_LOG_INFO("server", "listening on %s:%u (fd=%d)",
...
int ioh_server_run_once(ioh_server_t *srv, uint32_t timeout_ms)
{
    if (srv == nullptr) {
        return -EINVAL;
    }
    ...
    io_uring_for_each_cqe(ring, head, cqe)
    {
        ...
        if (op == IOH_OP_ACCEPT) {
            if (cqe->res >= 0) {
                ...
                ioh_conn_t *conn = ioh_conn_alloc(srv->pool);
                if (conn != nullptr) {
                    conn->fd = client_fd;
            ...
            if (conn->state == IOH_CONN_PROXY_HEADER) {
                ...
                if (proxy_ret > 0) {
                    ...
                    IOH_LOG_DEBUG("server", "conn %u: PROXY v%u from %s", conn->id,
            ...
            if (conn->recv_len > srv->config.max_header_size) {
                IOH_LOG_WARN("server", "conn %u: header too large (%zu > %u)", conn->id,
            ...
            if (consumed > 0) {
                ...
                if (req.content_length > srv->config.max_body_size) {
                    IOH_LOG_WARN("server", "conn %u: body too large (%zu > %u)", conn->id,
                ...
                if (req.content_length > 0 && body_avail < req.content_length) {
                    ...
                    if (srv->router != nullptr) {
                        ioh_route_match_t m =
                            ioh_router_dispatch(srv->router, req.method, req.path, req.path_len);
                        if (m.status == IOH_MATCH_FOUND && m.opts != nullptr) {
                            conn->route_body_timeout_ms = m.opts->body_timeout_ms;
                        }
                    }
            ...
            if (cqe->res < 0) {
                ...
                if (conn->send_offset < conn->send_len) {
                    ...
                    if (send_sqe != nullptr) {
                        ...
                        io_uring_sqe_set_data64(send_sqe, IOH_ENCODE_USERDATA(conn->id, IOH_OP_SEND));
                    } else {
                    ...
                    if (conn->state == IOH_CONN_TLS_HANDSHAKE) {
                        /* TLS handshake send done — need more recv */
                        (void)arm_recv(srv, conn);
                    } else if (conn->keep_alive && conn->state == IOH_CONN_HTTP_ACTIVE) {
            ...
            if (cqe->res == -ECANCELED) {
                ...
                ioh_conn_t *conn = find_conn_by_id(srv, conn_id);
                if (conn != nullptr) {
                    IOH_LOG_DEBUG("server", "conn %u: timeout, closing", conn->id);
                    (void)arm_close(srv, conn);
                }
            }
            ...
            if (cqe->res > 0) {
                ...
                IOH_LOG_INFO("server", "received signal %u, shutting down", signo);
                if (signo == SIGQUIT) {
            ...
            if (!srv->stopped && srv->signal_fd >= 0) {
                ...
                if (sig_sqe != nullptr) {
                    ...
                    io_uring_sqe_set_data64(sig_sqe, IOH_ENCODE_USERDATA(0, IOH_OP_SIGNAL));
                }
            }
        }
...
int ioh_server_shutdown(ioh_server_t *srv, ioh_shutdown_mode_t mode)
{
    ...
    IOH_LOG_INFO("server", "shutdown mode=%s, active=%u",
                mode == IOH_SHUTDOWN_DRAIN ? "drain" : "immediate", ioh_conn_pool_active(srv->pool));
    ...
    if (mode == IOH_SHUTDOWN_IMMEDIATE) {
         ...
         * ioh_server_run_once checks srv->stopped and returns early,
        ...
        while (ioh_conn_pool_active(srv->pool) > 0 && elapsed < drain_timeout_ms) {
            srv->stopped = false;
            (void)ioh_server_run_once(srv, DRAIN_POLL_MS);
            srv->stopped = true;
